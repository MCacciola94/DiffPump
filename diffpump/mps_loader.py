...
# warnings.warn("""pysmps: Backwards compatibility to pre-2.0 versions has been
#               abolished. Install older versions for code adjusted to the old
#               interface, e.g. via \r\n
...
class MPS:
    def __init__(self, defaults):
        self.defaults = {
            "Integer": {
                "type": "Integer",
                "lower": defaults["i_lower"],
                "upper": defaults["i_upper"],
            },
            "Continuous": {
                "type": "Continuous",
                "lower": defaults["c_lower"],
                "upper": defaults["c_upper"],
            },
        }
        self.name = "No Name"
        self.objectives = {}
    ...
    #    self.sos[name] = {
    #                  "degree": degree, "priority": priority, "variables": {}
    #                   }
    ...
    def add_variable(self, name, _type):
    ...
    def update_variable(self, variable, mod):
        self.variables[self.curr_bnd][variable].update(mod)

    def add_bnd_group(self, name):
        ...
        if len(self.variables) == 2:
            warnings.warn(
                ...
                + self.name
                + ". Switch boundary group using the"
                + "MPS.set_bounds_group function.",
                stacklevel=2,
            )
    ...
    def add_range(self, constr, value):
        if self.constraints[constr]["type"] == "G":
            self.ranges[self.curr_range][constr] = {
                "lower": 0,
                "upper": abs(value),
            }
            ...
            msg = "Range given for free row!"
            raise ValueError(msg)

    def add_rhs_group(self, name):
        if name in self.rhs:
            raise ValueError("The RHS group " + name + " already exists!")
        self.rhs[name] = {constr: 0 for constr in self.constraints}
        self.offsets[name] = {obj: 0 for obj in self.objectives}
        if len(self.rhs) == 2:
            warnings.warn(
...
def read_mps(path: Path, **kwargs):
    if not isinstance(path, Path):
        msg = "Path to .mps instance must be a Path object."
        raise TypeError(msg)
    if not path.exists():
        msg = f"Path {path} is not valid path to instance."
    ...
    given_bounds = {
        k: kwargs[k]
        for k in ["c_lower", "c_upper", "i_lower", "i_upper"]
        if k in kwargs
    }
    ...
    given_defaults = {
        k: kwargs[k] for k in ["MI_upper", "SC_lower"] if k in kwargs
    }
    ...
    with path.open() as reader:
        for line in reader:
            ...
            # SOS INSTRUCTION
                ...
                msg = "NETWORK is currently not implemented!"
                raise ValueError(msg)
                ...
                if line[0] == ROW_MODE_OBJ:
                    ...
                    mps.add_constraint(
                        line[1], {"type": line[0], "coefficients": {}}
                    )
            ...
            # COL MODE
                ...
                if line[0] not in mps._variables:
                    mps.add_variable(line[0], COL_TYPES[integral_marker])
                j = 1
                while j < len(line) - 1:
                    mps.set_coefficient(line[j], line[0], float(line[j + 1]))
                    j = j + 2
            ...
            # RHS MODE
                ...
                if mode == MODE[CORE_FILE_RHS_MODE_NAME_GIVEN]:
                    if line[0] != mps.get_curr_rhs():
                        msg = ("Other RHS name was given even though name"
                               " was set after RHS tag.")
                        raise ValueError(msg)
                elif line[0] not in mps.rhs:
                    mps.add_rhs_group(line[0])
            ...
            # SOS MODE
                ...
                warnings.warn(
                    "SOS functionality is currently not supported."
                    " If your program relies on Special Ordered Set"
                    " functionality the output might not be true"
                    " to the file input!",
                    stacklevel=2,
                )
            ...
            # BOUNDS MODE
                ...
                if mode == MODE[CORE_FILE_BOUNDS_MODE_NAME_GIVEN]:
                    if line[1] != mps.get_curr_bnd():
                        msg = (
                            "Other BND name was given even though"
                            " name was set after BND tag."
                        )
                        raise ValueError(msg)
                ...
                if line[0] == "UP":
                    mps.update_variable(line[2], {"upper": float(line[3])})
                elif line[0] == "UI":
                    mps.update_variable(
                        line[2], {"type": "Integer", "upper": float(line[3])}
                    )
                elif line[0] == "LO":
                    mps.update_variable(line[2], {"lower": float(line[3])})
                elif line[0] == "LI":
                    mps.update_variable(
                        line[2], {"type": "Integer", "lower": float(line[3])}
                    )
                elif line[0] == "FX":
                    mps.update_variable(
                        line[2],
                        {"lower": float(line[3]), "upper": float(line[3])},
                    )
                elif line[0] == "FR":
                    mps.update_variable(
                        line[2], {"lower": -math.inf, "upper": math.inf}
                    )
                elif line[0] == "MI":
                    mps.update_variable(
                        line[2],
                        {"lower": -math.inf, "upper": defaults["MI_upper"]},
                    )
                elif line[0] == "PL":
                    mps.update_variable(
                        line[2], {"lower": 0.0, "upper": math.inf}
                    )
                elif line[0] == "BV":
                    mps.update_variable(
                        line[2],
                        {"type": "Integer", "lower": 0.0, "upper": 1.0},
                    )
                elif line[0] == "SC":
                    ...
                    if len(line) > 3:
                        val = float(line[3])
                    mps.update_variable(
                        line[2], {"type": "Semi-Continuous", "lower": val}
                    )

            # RANGES MODE
            elif mode in [
                MODE[CORE_FILE_RANGES_MODE_NAME_GIVEN],
                MODE[CORE_FILE_RANGES_MODE_NO_NAME],
            ]:
                if mode == MODE[CORE_FILE_RANGES_MODE_NAME_GIVEN]:
                    if line[0] != mps.get_curr_range():
                        msg = (
                            "Other RANGES group tag was given even though"
                            " groupwas set after RANGES tag."
                        )
                        raise ValueError(msg)
                elif line[0] not in mps.ranges:
                    mps.add_range_group(line[0])
